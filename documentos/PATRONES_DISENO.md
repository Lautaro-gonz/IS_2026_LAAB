...
Cuando se crea un `Turno`, el sistema necesita reaccionar de múltiples formas simultáneas:

- Notificar al paciente por **email** que su turno fue creado
...
4. Cada observer decide qué hacer con esa información: `EmailObservador` imprime la notificación simulada al email, `SMSObservador` al teléfono, y `ConsoleObservador` registra el log interno.
...
```python
# El Subject — Turno suscribe y notifica sin conocer a los observers concretos
...
5. `turno.estado` es una `@property` que delega a `self._estado.nombre()`, devolviendo siempre el nombre del estado actual como string.
...
```python
# Estados concretos — cada uno encapsula sus propias reglas
...
| Transiciones inválidas | Condiciones mezcladas y propensas a olvidos | Cada estado lanza su propio `ValueError` descriptivo |
| Legibilidad | Difícil saber qué permite cada estado | Cada clase de estado es autoexplicativa |

### Interacción con Observer

Una de las ventajas del diseño es que State y Observer se complementan naturalmente. Cuando un estado concreto ejecuta una transición válida, llama `turno.notificar(mensaje)` como parte de la misma operación. Esto significa que **cada cambio de estado dispara automáticamente las notificaciones** a todos los observers suscritos, sin que ninguna de las dos capas sepa de la otra.
